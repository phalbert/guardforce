...
export const services = [
    ...
    { name: "Electronic Surveillance", path: "/surveillance" },
    { name: "Private Investigations", path: "/investigations" },
    { name: "Corporate Guards", path: "/guards" },
    { name: "Escort Services", path: "/escorts" },
    { name: "VIP Protection", path: "/vip-protection" },
    { name: "Smart Community Solutions", path: "/smart-solutions" },
]

export function ContractStrip() {
    return (
        <div className="fixed flex items-center justify-end bg-neutral text-white py-1 px-8 w-full z-20">
            <div className="flex space-x-2">
                <span>+256 709 910000</span>
                <span>guardforce20@gmail.com</span>
            </div>
        </div>
    );
}
...
export function Navbar() {
    ...
    return (
        <div className="navbar bg-base-100 pt-8 fixed z-10 px-24">
            ...
            <div className="flex-1">
                <a className="normal-case text-xl" href="/">Guardforce</a>
            </div>
            <div className="flex-1">
                <ul className="menu menu-horizontal p-0">
                    {menus.map(({ name, path }, idx) => (
                        <li tabIndex={idx} key={name} className="menu-item hover:bg-secondary hover:text-white">
...
export function Footer() {
    return (
        <div className="bottom-0 w-full px-24 bg-base-200">
            <footer className="footer py-6 text-base-content">
                <div>
                    <span className="footer-title">Services</span>
                    {services.map(({ name, path }) => (
                        <Link key={name} to={path} className="link link-hover">{name}</Link>
                    ))}
                </div>
                <div>
                    <span className="footer-title">Address</span>
                    <span>Plot 1477 Rubaga Road</span>
                    <span>P.O Box 31021 Clock Tower</span>
            ...
            <footer className="footer py-6 border-t text-base-content border-base-300">
                <div className="items-center grid-flow-col">
                    ...
                    <p>Guardforce Security and Investigation Services Ltd. <br /><i>Prevent and Protect</i></p>
                </div>
